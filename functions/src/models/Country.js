...
const countrySchema = new mongoose.Schema({
  ...
  continent: {
    ...
    enum: ['Asia', 'Africa', 'North America', 'South America', 'Antarctica', 'Europe', 'Australia/Oceania']
  },
  description: {
    type: String,
  ...
  climate: {
    type: String,
    enum: ['tropical', 'temperate', 'arid', 'polar', 'mediterranean', 'continental', 'subtropical']
  },
  bestTimeToVisit: [{
    season: {
      type: String,
  ...
  attractions: [{
    ...
    type: {
      type: String,
      enum: ['historical', 'natural', 'cultural', 'religious', 'adventure', 'urban']
    },
    description: String,
    location: {
      city: String,
...
countrySchema.methods.updateStatistics = async function() {
  ...
  if (stats.length > 0) {
    ...
    this.statistics.averageRating = Math.round(stats[0].averageRating * 10) / 10;
