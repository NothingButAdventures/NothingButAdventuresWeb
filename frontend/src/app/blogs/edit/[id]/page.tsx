...
export default function EditBlogPage({
    ...
    useEffect(() => {
        if (user && editor && !blogLoaded) {
            fetchBlog();
        }
    }, [user, editor, blogLoaded]);
    ...
    const fetchBlog = async () => {
        try {
            ...
            const response = await fetch(
                `${api.baseURL}/blogs/me/blogs/${resolvedParams.id}`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
            ...
            if (!response.ok) {
                alert("Blog not found or you don't have permission to edit it");
    ...
    const handleSubmit = async (publishStatus: "draft" | "published") => {
        ...
        try {
            ...
            const response = await fetch(
                `${api.baseURL}/blogs/${resolvedParams.id}`,
                {
                    method: "PATCH",
                    headers: {
    ...
    if (authChecking || !blogLoaded || !user) {
        return null;
    }
