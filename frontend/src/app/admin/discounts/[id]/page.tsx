...
export default function EditDiscountPage() {
    ...
    const editor = useEditor({
        extensions: [
            ...
            Placeholder.configure({
                placeholder: "Describe this discount... (Terms & conditions, Eligibility, etc.)",
            }),
            TextAlign.configure({
    ...
    const fetchDiscount = async () => {
        try {
            const res = await fetch(`${api.baseURL}/discounts/${id}`, {
                credentials: "include",
            });
            const data = await res.json();
            if (data.status === "success") {
                const disc = data.data.discount;
                setDiscount(disc);
                setName(disc.name);
                setPercentage(disc.percentage?.toString() || "");
                setShortDescription(disc.shortDescription || "");
                setColor(disc.color || "#22C55E");
                setImage(disc.image || "");
                setIcon(disc.icon || "");
                setIsActive(disc.isActive);
                setValidFrom(disc.validFrom ? new Date(disc.validFrom).toISOString().split("T")[0] : "");
                setValidUntil(disc.validUntil ? new Date(disc.validUntil).toISOString().split("T")[0] : "");
                editor?.commands.setContent(disc.description || "");
            }
        } catch (err) {
    ...
    const uploadImageToSupabase = async (file: File): Promise<string | null> => {
        try {
            return await uploadDiscountImage(file);
        } catch (error) {
    ...
    const handleSave = async (e?: React.FormEvent) => {
        e?.preventDefault();
        if (!name || !percentage) return alert("Name and percentage are required");
    ...
    if (loading) return <div className="p-10 flex justify-center text-zinc-500 animate-pulse">Loading discount...</div>;
    if (!discount) return <div className="p-10 text-center">Discount not found</div>;
                ...
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Link href="/admin/discounts" className="p-2 border border-gray-200 hover:bg-zinc-100 rounded-md transition text-zinc-600 bg-white shadow-sm flex items-center justify-center">
                                <Icons.Back className="w-4 h-4" />
                            </Link>
                            <div>
                                <h1 className="text-xl font-bold text-zinc-800">Edit Discount</h1>
                                <p className="text-xs text-gray-500">Update details for {discount.name}</p>
                            </div>
                        </div>
                        <button
                            onClick={handleSave}
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                ...
                <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6 md:p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-1">Discount Name</label>
                                <input
                                    type="text"
                                    value={name}
                            ...
                            <div className="flex items-center gap-2 pt-2">
                                ...
                                />
                                <label htmlFor="isActive" className="text-sm font-medium text-zinc-700">Active (visible in tour discount options)</label>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-1">Valid From (optional)</label>
                                <input
                                    type="date"
                                    value={validFrom}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-1">Valid Until (optional)</label>
                                <input
                                    type="date"
                                    value={validUntil}
                ...
                <div className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-gray-50/50">
                        <h2 className="text-lg font-semibold text-zinc-800">Terms & Conditions</h2>
                        {/* Simple Toolbar */}
