...
export default function EditPhysicalRatingPage() {
    const params = useParams();
    const router = useRouter();
    const id = params?.id as string;

    const [physicalRating, setPhysicalRating] = useState<PhysicalRating | null>(null);
    ...
    const editor = useEditor({
        extensions: [
            ...
            Placeholder.configure({
                placeholder: "Describe this physical rating level... (What activities, fitness requirements...)",
            }),
            TextAlign.configure({
    ...
    useEffect(() => {
        if (id) fetchPhysicalRating();
    }, [id]);

    const fetchPhysicalRating = async () => {
        try {
            const res = await fetch(`${api.baseURL}/physical-ratings/${id}`, {
                credentials: "include",
            });
            const data = await res.json();
            if (data.status === "success") {
                const rating = data.data.physicalRating;
                setPhysicalRating(rating);
                setName(rating.name);
                setLevel(rating.level || 1);
                setShortDescription(rating.shortDescription || "");
                setColor(rating.color || "#3B82F6");
                setIcon(rating.icon || "");
                setIsActive(rating.isActive);
                editor?.commands.setContent(rating.description || "");
            }
        } catch (err) {
            console.error("Error fetching physical rating:", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.status === "success") {
                alert("Physical rating saved successfully!");
                router.refresh();
            } else {
