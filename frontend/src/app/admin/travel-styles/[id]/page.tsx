...
import { uploadTravelStyleImage } from "@/lib/firebase";
...
export default function EditTravelStylePage() {
    ...
    const editor = useEditor({
        extensions: [
            ...
            Placeholder.configure({
                placeholder: "Describe this travel style... (What makes it unique, Target audience, Best for...)",
            }),
            TextAlign.configure({
    ...
    const fetchTravelStyle = async () => {
        try {
            ...
            if (data.status === "success") {
                const style = data.data.travelStyle;
                ...
                setIcon(style.icon || "");
                setIsActive(style.isActive);
                editor?.commands.setContent(style.description || "");
            }
        } catch (err) {
            console.error("Error fetching travel style:", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const uploadImageToSupabase = async (file: File): Promise<string | null> => {
        try {
            return await uploadTravelStyleImage(file);
        } catch (error) {
    ...
    if (loading) return <div className="p-10 flex justify-center text-gray-500 animate-pulse">Loading travel style...</div>;
    if (!travelStyle) return <div className="p-10 text-center">Travel style not found</div>;
                ...
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <Link href="/admin/travel-styles" className="p-2 hover:bg-gray-100 rounded-full transition text-gray-500">
                                <Icons.Back className="w-5 h-5" />
                            </Link>
                            <div>
                                <h1 className="text-xl font-bold text-gray-900">Edit Travel Style</h1>
                                <p className="text-sm text-gray-500">Update details for {travelStyle.name}</p>
                            </div>
                        </div>
                        <button
                            onClick={handleSave}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Travel Style Name</label>
                                <input
                                    type="text"
                                    value={name}
                    ...
                    <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50 hover:bg-gray-50 transition-colors">
                        {image ? (
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-gray-500 mb-4">Upload a high-quality cover image for this travel style</p>
                            </div>
                        )}
